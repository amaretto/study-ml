...
func main() {
	...
	flag.StringVar(&dir, "dir", filepath.Join(filepath.SplitList(build.Default.GOPATH)[0], "src/github.com/amaretto/ml-study"), "Directory containing the trained model and labels files")
	...
	for float64(probabilities[i]) > probability {
		...
		drawString(
			...
			fmt.Sprintf("%s(%3.0f%%)", labels[idx], probabilities[idx]*100.0))
		i++
	}
